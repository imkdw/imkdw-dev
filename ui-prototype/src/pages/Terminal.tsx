...
const Terminal = () => {
  ...
  const handleCommand = (cmd: string) => {
    ...
    if (trimmedCmd === '') {
      ...
      output = commands[baseCommand as keyof typeof commands].execute();
    } else {
      output = [
        `명령어를 찾을 수 없습니다: ${baseCommand}`,
        '"help"를 입력하여 사용 가능한 명령어를 확인하세요.'
      ];
    }
  ...
  return (
    <div className={`${isFullscreen ? 'fixed inset-0 z-50' : 'container mx-auto px-2 py-2 md:px-4 md:py-8'}`}>
      <div className={`bg-black text-green-400 font-mono text-sm ${isFullscreen ? 'h-full' : 'h-[calc(100vh-2rem)] md:h-[600px]'} rounded-lg border border-green-400/30 shadow-2xl overflow-hidden`}>
        {/* 터미널 헤더 */}
        <div className="bg-gray-900 px-4 py-2 flex items-center justify-between border-b border-green-400/30">
          <div className="flex items-center space-x-2">
            <TerminalIcon className="h-4 w-4 text-green-400" />
            <span className="text-green-400 text-xs">DevBlog Terminal</span>
          </div>
          <div className="flex items-center space-x-2">
            <Button
              variant="ghost"
        ...
        >
          ...
          {history.map((entry, index) => (
            <div key={index} className="space-y-1">
              {entry.command && (
                <div className="flex items-center space-x-2">
                  <span className="text-blue-400">user@devblog:~$</span>
