...
export const getSeriesDetail = withErrorHandling(async (slug: string) => {
  return getApiClient().get<ISeriesDetailDto>(buildEndpoint('GET_SERIES_DETAIL', { slug }));
});

export const createSeries = withErrorHandling(async (data: ICreateSeriesDto) => {
  return getApiClient().post<ICreateSeriesDto, IResponseCreateSeriesDto>(buildEndpoint('CREATE_SERIES'), data);
});

export const updateSeries = withErrorHandling(async (slug: string, data: IUpdateSeriesDto) => {
  return getApiClient().patch<IUpdateSeriesDto>(buildEndpoint('UPDATE_SERIES', { slug }), data);
});

export const deleteSeries = withErrorHandling(async (slug: string) => {
  return getApiClient().delete(buildEndpoint('DELETE_SERIES', { slug }));
});
